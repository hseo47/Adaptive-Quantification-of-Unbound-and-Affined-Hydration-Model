...
n_units = 10  # <-- Modify as needed
E_PDMS = 2.0  # <-- Modify as needed
filename_suffix = f"n{n_units}_E{E_PDMS:.1f}".replace('.', '_')

output_folder = f"output_n{n_units}_E{E_PDMS:.1f}".replace('.', '_')
hydration_path = os.path.join(base_dir, output_folder, f"{filename_suffix}_hydration_map.csv")
...
# === Parse voxel dimensions and max hydration from distrib file ===
...
# Save and show
output_path = os.path.join(base_dir, output_folder, f"{filename_suffix}_shielding_radius_fit.png")
